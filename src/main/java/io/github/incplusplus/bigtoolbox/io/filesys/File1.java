...
public class File1 extends java.io.File implements Iterable<File1> {
  ...
  private List<StringBuilder> renderDirectoryTreeLines(File1 tree) {
    ...
    Iterator<File1> sortedIterator =
        tree.children.stream().sorted().collect(Collectors.toList()).iterator();
    while (sortedIterator.hasNext()) {
      List<StringBuilder> subtree = renderDirectoryTreeLines(sortedIterator.next());
