...
public class Directory extends Entry {
  ...
  // Assume that changes could happen and the directory should be re-indexed before attempting each
  // action
  ...
  private final String[] filesToIgnoreBuiltIn = {
    "Directory.class", "File.class", "FileRelatedTools.class"
  };
  ...
  public Directory(
      boolean assumeChangesCouldHappen, File path, String[] filesToIgnore, boolean verbose) {
  ...
  void index(boolean override) {
    ...
    for (File i : (localContents != null) ? localContents : new File[0]) {
      if (i.isDirectory()) {
        ...
        Directory tempDir =
            new Directory(
                assumeChangesCouldHappen,
                i,
                excludedFiles.toArray(new String[excludedFiles.size()]),
                verbose);
        tempDir.index();
    ...
    for (Entry i : in.getContents()) {
      if (i instanceof Directory) {
        if (verbose && currentLargestFile.hasNoContents()) {
          System.out.println(
              "Sent a getLargest() with no contents from "
        ...
        if (verbose) {
          System.out.println(
              largestInDirectory.getName()
                  + ": "
                  + largestInDirectory.getSize()
                  + " is a directory");
        }
        if (currentLargestFile.hasNoContents()) {
          ...
          if (verbose) {
            System.out.println(
                "currentLargestFile was "
                    + currentLargestFile.getName()
                    + ": "
                    + currentLargestFile.getSize()
                    + " but is now "
                    + largestInDirectory.getName()
                    + ": "
                    + largestInDirectory.getSize());
